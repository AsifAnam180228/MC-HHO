...
class Graph:
    ...
    def get_partition_cost(self):
        ...
        for edge in self.edges:
            if edge.left_v.partition_label == 'A':
                s1.append(edge.left_id)
            else:
                s2.append(edge.left_id)

            if edge.right_v.partition_label == 'A':
                s1.append(edge.right_id)
            else:
                s2.append(edge.right_id)
        ...
        s1 = list(set(s1))
        s2 = list(set(s2))
        subset = [0 for _ in range(len(s1) + len(s2))]
...
class KernighanLin:
    ...
    def partition(self):
        ...
        return self.graph.get_partition_cost()


def graph_partition(filename):
...
if __name__ == "__main__":
    graph_partition(filename='Gset/G1.txt')
